 ...
 * Real RugCheck API + Alchemy RPC + USDC payment verification
 */
...
async function scoreTok(mintAddress) {
  ...
  // Try RugCheck API first (most comprehensive)
  ...
  } catch (e) { /* fallback to RPC */ }

  // Always get RPC data for additional checks
  let rpcData = {};
  try {
    rpcData = await getRPCData(mintAddress);
  } catch (e) { /* partial data ok */ }
